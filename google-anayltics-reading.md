...
For now, we will keep all of our sites (FS, JS and flex projects) under one single property, since it's easier to track.
...
To counter that, GA allows us to filter traffic by IP. That will be our last step, as it's important to keep us unfiltered while we test the tracking code set up.
